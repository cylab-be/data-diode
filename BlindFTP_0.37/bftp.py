...
# 27/10/2008 v0.24  LV:- V�rification pr� emission de stabilit�
#                      - Correction boucle �mission limit�e � MinFileRedundancy et non MinFileRedundancy+1
#                      - Correction synchro arbo mode boucle (plus de sortie sur aucun fichier � emetre)
#                      - Correction affichage de l'aide (UnicodeError) - "reprise �/a chaud"
                ...
                # Liste vide : rien � transmettre
                    ...
                    if options.boucle:
                        ...
                        if attente > 0:
                            ...
                            # DIODE BEGIN
                            print("Tous les fichiers ont �t� envoy�s !")
                            db_uploaders_clie.add(options.usage, '0', 0)
                            #DIODE END
                            time.sleep(attente)
            Console.Print_temp("%s - Sauvegarde du fichier de reprise" %mtime2str(time.time()))
            DRef.et.set(xfl.ATTR_TIME, str(time.time()))
...
def analyse_options():
    ...
    parseur.add_option("-x", "--expurge", action="store_true", dest="expurge_repertoire",\
        default=False, help="Supprimer les fichiers et dossiers envoy�s (ne pas utiliser avec -S)")
    parseur.add_option("-u", dest="usage",\
        help="Pr�ciser l'usage qui sera retenu dans la base de donn�es (exclusif au projet Date Diode ERM)", type="str", default="ftp")
    # DIODE END
